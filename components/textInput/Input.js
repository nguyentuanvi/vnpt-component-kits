...
export default class Input extends React.PureComponent {
    constructor(props) {
        super(props);
        const { value, bankCardType } = this.getText(
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (
            ...
            let valueFormatted = removeAccent
                ? ValueUtil.removeAlias(get(nextProps, 'value', ''))
                : get(nextProps, 'value', '');
            let bankCardType = {};
            if (
                isFormatBankCard &&
                (inputType === 'bankCard' ||
                    bankCardArray.indexOf(valueFormatted?.substr(0, 4)) >= 0)
            ) {
                valueFormatted = NumberUtils.formatBankCardNumber(
                    valueFormatted,
                    maxLength,
                );
                bankCardType = NumberUtils.checkCardType(
                    valueFormatted,
                    regexData,
                );
            } else if (inputType === 'money' && valueFormatted?.length > 0) {
                valueFormatted = ValueUtil.removeSymbols(valueFormatted);
                const positiveNumberInput = positiveNumber
                    ? Math.abs(
                          NumberUtils.formatMoneyToNumber(
                              valueFormatted,
                              currencyUnit,
                          ),
                      )
                ...
                valueFormatted = NumberUtils.formatNumberToMoney(
                    positiveNumberInput,
                    currencyUnit,
                );
            } else if (inputType === 'date' && valueFormatted?.length > 0) {
                valueFormatted = DatetimeUtils.stringToDate(
                    valueFormatted,
                    formatDate,
                    this.backup,
                );
    ...
    onLeftIconPress = () => {
        ...
        if (onLeftIconPress && typeof onLeftIconPress === 'function')
            onLeftIconPress();
    };

    renderLetters = () => {
        const { maxLength = PASSWORD_MAX_LENGTH_DEFAULT, textDotStyle } =
            this.props;
        const { value } = this.state;
        ...
        for (let i = 0; i < maxLength; i += 1) {
            textUnderlines.push(
                i < value.length ? (
                    <Text key={`k_${i}`} style={[styles.textDot, textDotStyle]}>
                        {value[i]}
                    </Text>
                ) : (
                    <View
    ...
    getText = (text = '', isDefault) => {
        ...
        let bankCardType = {};
        if (
            (inputType === 'percent' || inputType === 'money') &&
            text?.length > 0
        ) {
            text =
                inputType === 'percent' ? text : ValueUtil.removeSymbols(text);
            if (
                !text.endsWith(currencyUnit) &&
                text?.length >= 1 &&
                !isDefault &&
                this.cacheValue?.length > 0
            ) {
            ...
            const positiveNumberInput = positiveNumber
                ? Math.abs(NumberUtils.formatMoneyToNumber(text, currencyUnit))
                : NumberUtils.formatMoneyToNumber(text, currencyUnit);
            valueFormatted =
                inputType === 'percent'
                    ? NumberUtils.formatNumberToPercent(
                          positiveNumberInput,
                          currencyUnit,
                      )
            ...
            if (
                valueFormatted?.length > maxLength + currencyUnit?.length &&
                Platform.OS === 'android' &&
                !isDefault
            ) {
                const { value: newText } = this.getText(
                    valueFormatted.slice(
                        0,
                        maxLength + currencyUnit?.length - 1,
                    ),
                );
                valueFormatted = newText;
            }
            if (
                (positiveNumberInput === 0 || positiveNumberInput === '') &&
                !isDefault
            )
                valueFormatted = '';
            ...
            text?.length > 0 &&
            (inputType === 'bankCard' ||
                bankCardArray.indexOf(text?.substr(0, 4)) >= 0)
        ) {
    ...
    validateText = (text) => {
        ...
        if (regex && text?.length > 0) {
            try {
                ...
                console.log(
                    'Something is incorrect, please check REG operator',
                );
            }
        }
    };
        ...
        this.setState({ focused: false }, () => {
            ...
            if (regexOnBlur) {
                ...
                this.setState({ validatedOnBlur: result }, () => {
                    if (typeof onValidateOnBlur === 'function')
                        onValidateOnBlur(result);
                });
            }
    ...
    getSelectionInput = () => {
        ...
        const defaultSelection =
            value?.length - currencyUnit?.length > 0
                ? value?.length - currencyUnit?.length
                : 0;
        const maxSelection = maxLength ? maxLength + currencyUnit?.length : 0;
        const selectionPosition =
            maxSelection && defaultSelection > maxSelection
                ? maxSelection
                : defaultSelection;
    ...
    getMaxLength = () => {
        ...
        return maxLength
            ? {
                  maxLength:
                      inputType === 'money'
                          ? maxLength + currencyUnit?.length || 0
                          : maxLength,
              }
            : {};
    };
    ...
    renderInput = (isInput = true) => {
        const {
            ...
            focusBorderColor,
            errorMessage = null,
            useErrorMessageInRegex,
            rightComponent,
            hideErrorMessageIcon,
            showCardType,
            focusStyle,
            regexOnBlur,
            errorMessageOnBlur,
            ...
            secureTextEntry,
            initWithDots,
            floatingNumberOfLines,
            isShowCancel,
            suggestionText,
            onSuggestionItemPress,
            titleStyle,
            subTitleStyle,
        ...
        const hasRightView =
            ...
            errorMessage ||
            (cancellable && value && value?.length > 0) ||
            icon;
        ...
        if (
            ((useErrorMessageInRegex && !validated) ||
                !useErrorMessageInRegex) &&
            errorMessage
        ) {
        ...
        return (
            <View>
                <View
                    accessibilityLabel={`${floatingValue || ''}/Input/${
                        isInput ? 'Typing' : 'Click'
                    }`}
                    style={textFlex ? { flex: textFlex } : {}}>
                    <View
                        style={[
                            styles.container,
                            style,
                            (!validated && regex) ||
                            (errorMessage && !useErrorMessageInRegex) ||
                            (!focused && regexOnBlur && !validatedOnBlur)
                                ? { borderColor: Colors.red_04 }
                                : {},
                            !errorMessage && validated && focused
                                ? simpleFocusStyle
                                : {},
                            focused ? focusStyle : {},
                            disabled
                        ...
                        <View style={[styles.content, contentContainerStyle]}>
                            {leftIcon ? (
                                <TouchableOpacity
                                    ...
                                    disabled={
                                        typeof onLeftIconPress !== 'function'
                                    }
                                    hitSlop={hitSlop}>
                                    <Image
                                        source={leftIcon}
                                        style={[
                                            ...
                                            ValueUtil.mergeStyle(leftIconStyle),
                                        ]}
                                    />
                                </TouchableOpacity>
                            ) : null}
                            ...
                            <View
                                ...
                                //     Colors.black_02,
                                //     Colors.black_02,
                                //     disabled ? Colors.black_02 : Colors.white,
                                // ]}
                                style={styles.floatingView}>
                                <TouchableOpacity
                                    disabled={!onPressFloating}
                                    onPress={onPressFloating}
                                    style={{ flexDirection: 'row'}}>
                                    <Text.SubTitle
                                        numberOfLines={floatingNumberOfLines}
                                        style={[
                                            styles.floatingStyle,
                                            ValueUtil.mergeStyle(floatingStyle),
                                        ]}>
                                        {floatingValue}
                                    </Text.SubTitle>
                                    {floatingIcon && (
                            ...
                            {isInput && !disabled ? (
                                ...
                                <View style={{width:'100%', paddingBottom:5}}>
                                    ...
                                    selectionColor={Colors.pink_05_b}
                                    autoCapitalize="none"
                                    ...
                                    style={[
                                        ...
                                        textFlex
                                            ? { flex: textFlex }
                                            : { minWidth: 20 },
                                        disabled && { color: Colors.black_09 },
                                    ]}
                                    onChangeText={this.onChangeText}
                                    ref={(ref) => (this.textInput = ref)}
                                    numberOfLines={1}
                                    secureTextEntry={
                                        secureTextEntry && !initWithDots
                                    }
                                    caretHidden={
                            ...
                            <View style={hasRightView ? styles.rightIcons : {}}>
                                {bankCardType?.cardUrl && showCardType ? (
                                    <Image
                                        ...
                                        source={{ uri: bankCardType?.cardUrl }}
                                    />
                                ) : (
                                    <View />
                                )}
                                {(regex || (regexOnBlur && !focused)) &&
                                value?.length > 0 &&
                                showCheckIcon ? (
                                    <Image
                                        source={
                                            validated && validatedOnBlur
                                                ? validatedIcon
                                                : validatedFailedIcon
                                        }
                            ...
                            <Text.Title style={[styles.currency, mergeStyle(currencyStyle)]}>
                                ...
                                {(cancellable &&
                                    ...
                                    value?.length > 0 &&
                                    focused) ||
                                (value && value?.length > 0 && isShowCancel) ? (
                                    <TouchableOpacity
                                        accessibilityLabel={`${
                                            floatingValue || ''
                                        }/Input/'Click'/Clear}`}
                                ...
                                {icon ? (
                                    <TouchableOpacity
                                        ...
                                        <Image
                                            ...
                                            style={[
                                                styles.iconDefault,
                                                ValueUtil.mergeStyle(iconStyle),
                                            ]}
                                        />
                                    </TouchableOpacity>
                                ) : null}
                    ...
                    {typeof errorMessageDisplay === 'string' &&
                    !hideMessageError ? (
                        <View
                            style={[
                                styles.errorMessageView,
                                errorMessageContainerStyle,
                            ]}>
                            <Text.SubTitle style={styles.errorMessage}>
                                {errorMessageDisplay || ' '}
                            </Text.SubTitle>
                        </View>
                    ) : (
...
Input.propTypes = {
    ...
    regexOnBlur: PropTypes.oneOfType([
        PropTypes.instanceOf(RegExp),
        PropTypes.string,
    ]),
...
Input.defaultProps = {
    ...
    type: 'medium',
    focusBorderColor: Colors.pink_05_b,
